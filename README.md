# BlazorB2C

## Azure AD B2C Documentation
- [Access Tokens in Azure Active Directory B2C](https://learn.microsoft.com/en-us/azure/active-directory-b2c/access-tokens)
- [Get started with Microsoft Graph in Azure AD B2C](https://learn.microsoft.com/en-us/azure/active-directory-b2c/microsoft-graph-get-started?tabs=app-reg-ga)

## Microsoft Graph Resources
- [Get access without a user (Microsoft Graph authentication)](https://learn.microsoft.com/en-us/graph/auth-v2-user?tabs=http)
- [Use the Microsoft Graph API](https://learn.microsoft.com/en-us/graph/use-the-api)
- [Microsoft Graph Overview](https://learn.microsoft.com/en-us/graph/overview)
- [Microsoft Graph Explorer](https://developer.microsoft.com/en-us/graph/graph-explorer)
- [Microsoft Graph Gallery (Samples & SDKs)](https://developer.microsoft.com/en-us/graph/gallery/?filterBy=Samples,SDKs)

## Blazor Authentication Resources
- [Azure Samples: Microsoft Identity with Blazor Server](https://github.com/Azure-Samples/ms-identity-blazor-server)
- [Blazor Help Website: ViewBlogPost/55](https://blazorhelpwebsite.com/ViewBlogPost/55)

## Tools & Utilities
- [Microsoft Identity Web App Provisioning Tool](https://github.com/AzureAD/microsoft-identity-web/blob/master/tools/app-provisioning-tool/vs2019-16.9-how-to-use.md)

## Known Issues & Workarounds
- [.NET 8 Upgrade Post-Logout Loop in Blazor Server App with Azure AD B2C](https://github.com/dotnet/aspnetcore/issues/52245)

## Additional Resources
- [Azure AD Change Management Simplified](https://techcommunity.microsoft.com/blog/microsoft-entra-blog/azure-ad-change-management-simplified/2967456)

## Postman Resources
- [Calling an Azure AD Secured API with Postman](https://dev.to/425show/calling-an-azure-ad-secured-api-with-postman-22co)
- [Setting Azure AD B2C Authentication in Postman](https://akshaybabloo.medium.com/setting-azure-ad-b2c-authentication-in-postman-b8e7e1e48513)
- [Request Access Token in Postman for Azure AD B2C](https://stackoverflow.com/questions/47275079/request-access-token-in-postman-for-azure-ad-b2c)

## Token Verification Tools
- [JWT.MS - JWT Decoder](https://jwt.ms/)

https://learn.microsoft.com/en-us/entra/identity-platform/scenario-web-api-call-api-app-configuration?tabs=aspnetcore

https://github.com/Azure-Samples/ms-identity-docs-code-dotnet/tree/main